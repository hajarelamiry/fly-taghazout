      ...
      >
        <div className="container flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2">
            <img src={logo} alt="Fly Taghazout" className="h-10 w-auto" />
          </Link>
            ...
            {navItems.map((item) => (
              <Link
                ...
                className={cn(
                  ...
                  location.pathname === item.href ? "text-primary" : scrolled ? "text-foreground" : "text-primary-foreground"
                )}
              >
                {item.label}
              </Link>
          ...
          <div className="hidden lg:flex items-center gap-3">
            <Button variant="navCta" size="default" asChild>
              <Link to="/reserver">Réserver votre vol</Link>
            </Button>
          </div>
